...
def fpgrowth_algo(corpus_list):
    start_time = time.time()
    fp_itemsets = fpgrowth(encode_onehot(corpus_list), min_support=0.003, use_colnames=True)
    print("---Runtime FP Growth: %s seconds ---" % (time.time() - start_time))
    fp_itemsets['length'] = fp_itemsets['itemsets'].apply(lambda x: len(x))
    print("the number of frequent itemsets generated by FP Growth:", len(fp_itemsets))
    # generate_rules(fp_itemsets)
    return fp_itemsets
# Create corpus list
def create_corpus(dataset,  concern: bool = False, app: bool = False):
...
if __name__ == '__main__':
    ...
    apriori_rules = apriori_algo(create_corpus(dataset, concern=True, app=True))
