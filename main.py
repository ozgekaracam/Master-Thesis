...
def generate_rules(frequent_itemsets):
    ...
    filtered_rules.antecedents = filtered_rules.antecedents.apply(lambda x: next(iter(x)))
    filtered_rules.consequents = filtered_rules.consequents.apply(lambda x: next(iter(x)))
...
def encode_onehot(corpus_list):
    ...
    #calculate_support(corpus_df)
    return corpus_df
# Apriori algorithm
def apriori_algo(corpus_list):
    start_time = time.time()
    # Set max_len = 2 for only top rules
    frequent_itemsets = apriori(encode_onehot(corpus_list), min_support=0.003, use_colnames=True, max_len=2, low_memory=True)
...
def fpgrowth_algo(corpus_list):
    ...
    return rules
def calculate_support(corpus_df):
    ...
    plt.bar(top_50_words['Word'], top_50_words['Support'])
    plt.xlabel('Words')
    plt.ylabel('Support')
